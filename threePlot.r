setwd("~/Dropbox/SciLifeLab/PROJECTS/andreas_projects/genespider-paper/data/GS_fig_141116_nodiag/andreas")
...
setwd("~/Dropbox/SciLifeLab/PROJECTS/andreas_projects/genespider-paper/data/GS_fig_141116_nodiag/finalFIG")
...
for (i in 1:length(ldf_1)){
 ...
 # M1$method = factor(M1$method, levels = levels(M1$method)[c(1,4,3,2)])
  ...
  for (x in 1:(n)){
    #TP
    M2$y1[x]<- m1*x + (M2$TP[n-q])
    #TN
    M2$y2[x]<- m2*x + (M2$TN[n-r])#+M2$TP[n-q])
    #FP
    M2$y3[x]<- m3*x + (M2$FP[n-s])#+M2$TP[n-r]+M2$TP[n-q])
    #FN
    M2$y4[x]<- m4*x + (M2$FN[n-t])#+M2$TP[n-r]+M2$FP[n-s]+M2$TP[n-q])
    M2$MCC[x]<- (M2$y1[x] * M2$y2[x] - M2$y3[x]* M2$y4[x])/ sqrt((M2$y1[x]+M2$y3[x])*(M2$y1[x]+M2$y4[x])*(M2$y2[x]+M2$y3[x])*(M2$y2[x]+M2$y4[x]))
  } 
  #MCC
  M6<-data.frame(c(M1$MCC,M2$MCC[1:(length(M2$MCC))]))
  M6$MCC<-(c(M1$MCC,0,M2$MCC[2:(length(M2$MCC)-1)],0))
  M6$zeta<-c(M1$zeta,M1$zeta[(100+2*n):(3*n+99)])
  M6$method<-as.factor(c(M1$method,rep("ARACNe_null",each=n)))
  
  M6$method = factor(M6$method, levels = levels(M6$method)[c(2,1,5,4,3)])
  #M1$methodd = factor(M6$method, levels = levels(M6$method)[c(2,5,4,3)])
  #P2<-ggplot(data=M1,aes(x=M1$zeta,y=MCC,group=method)) + geom_line(aes(linetype=method,colour=method))+geom_point(aes(color=method)) + scale_colour_manual(values = my.cols) +xlab("Zetavec") +guides(colour = guide_legend(reverse=T),linetype = guide_legend(reverse=T)) 
  P2<-ggplot(data=M6,aes(x=M6$zeta,y=MCC,group=(method))) + geom_line(aes(linetype=method,colour=method))+geom_point(aes(color=method))+scale_colour_manual(values = (my.cols)) +ylim(-1,1) +xlab("Zetavec") + theme(legend.direction="horizontal") +guides(colour = guide_legend(reverse=T),linetype = guide_legend(reverse=T)) 
  
  #Sparsity
  P3<-ggplot(data=M1,aes(x=M1$zeta,y=1-(nlinks/(n)),group=method))  + geom_line(aes(linetype=method,colour=method))+geom_point(aes(color=method)) + scale_colour_manual(values = my.cols)+xlab("Zetavec")+ylab("Sparsity") + theme(legend.direction="horizontal") +guides(colour = guide_legend(reverse=T),linetype = guide_legend(reverse=T))
